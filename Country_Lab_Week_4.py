...
def display_menu():
    print("COMMAND MENU")
    print("view   - View country name")     #Command line and prompts
...
def display_codes(countries):
    codes = list(countries.keys())          #Why are there two
    codes.sort()
    codes_line = "Country codes:  "
    for code in codes:                      #for loop starts 
        codes_line += code + " "
    print(codes_line)
...
def main():
    ...
    while True:
        ...
        command = command.lower()              #redundant 
        if command == "view":
            view(countries)
        elif command == "add":
            add(countries)
